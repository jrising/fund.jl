...
function timestep(s::temperaturemacroeconomic, t::Int)
    ...
    if t == 1
        for r in d.regions
            ...
            v.baseline[r] = sum(p.macrocoeffs .* [p.regtmp[1, r], p.regtmp[1, r] * p.temp90[r]])
        end
    else
        for r in d.regions
            v.changeypcgrowth[t, r] = exp(sum(p.macrocoeffs .* [p.regtmp[t, r], p.regtmp[t, r] * p.temp90[r]]) - v.baseline[r])
        end
    end
end
