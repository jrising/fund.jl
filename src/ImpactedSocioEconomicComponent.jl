...
@defcomp impactedsocioeconomic begin
	regions = Index()
    ...
    changeypcgrowth = Parameter(index=[time, regions])
...
function timestep(s::impactedsocioeconomic, t::Int)
    v = s.Variables
    p = s.Parameters
    d = s.Dimensions
    ...
    if t==1
        ...
        for r in d.regions
            v.consumption[t, r] = max(v.income[t, r] * 1000000000.0 * (1.0 - p.savingsrate),0.0) # Dropped eloss and sloss
        end
        v.globalconsumption[t] = sum(v.consumption[t,:])
